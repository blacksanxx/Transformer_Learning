...
model_save_path = "./module/BaseLine_model_Picture.pth"  # --- 添加模型保存路径 ---
...
def train_epoch(model, dataloader, optimizer, criterion, device):
    ...
    for batch in tqdm(dataloader, desc="Training"):
        ...
        # 计算准确率和 token 数（忽略 padding）
        with torch.no_grad():
            preds = outputs.argmax(dim=-1).view(-1)  # [B*T]
            labels = tgt_label_ids.view(-1)          # [B*T]
            mask = (labels != -100)                  # 有效 token 掩码
            correct = (preds == labels) & mask
            total_correct += correct.sum().item()
            total_tokens += mask.sum().item()
            total_loss += loss.item() * mask.sum().item()  # 加权损失（可选，也可用 batch 平均）

    avg_loss = total_loss / total_tokens if total_tokens > 0 else 0
    acc = total_correct / total_tokens if total_tokens > 0 else 0
    ppl = math.exp(avg_loss) if avg_loss < 700 else float('inf')  # 防止 exp 溢出
...
def main():
    ...
    for epoch in range(num_epochs):
        ...
        train_loss, train_acc, train_ppl = train_epoch(model, train_dataloader, optimizer, criterion, device)
        ...
        if val_loss < best_val_loss:
            ...
            print(f"Model not saved. Best Val Loss: {best_val_loss:.4f}")
    ...
    # Loss
    axes[0].plot(epochs_list, train_losses, label='Train Loss', marker='o')
    axes[0].plot(epochs_list, val_losses, label='Val Loss', marker='s')
    ...
    # Accuracy
    axes[1].plot(epochs_list, train_accs, label='Train Acc', marker='o')
    axes[1].plot(epochs_list, val_accs, label='Val Acc', marker='s')
    ...
    # Perplexity
    axes[2].plot(epochs_list, train_ppls, label='Train PPL', marker='o')
    axes[2].plot(epochs_list, val_ppls, label='Val PPL', marker='s')
    axes[2].set_title('Perplexity')
    axes[2].set_xlabel('Epoch')
    axes[2].set_ylabel('PPL')
    axes[2].set_yscale('log')  # PPL 通常用对数坐标更清晰
    axes[2].legend()
    axes[2].grid(True)
    ...
    plt.show()
    print(f"Training curves (Loss, Accuracy, Perplexity) saved to {plot_save_path}")
